...
def get_imagenet_classes():
    """دانلود اسامی واقعی ۱۰۰۰ کلاس ImageNet برای نمایش خوانا"""
    ...
    if not os.path.exists(file_path):
        try:
            print("📥 در حال دریافت فایل اسامی واقعی ۱۰۰۰ کلاس ImageNet...")
            urllib.request.urlretrieve(url, file_path)
        except Exception:
            # در صورت عدم دسترسی به اینترنت، اسامی پیش‌فرض قرار می‌گیرند
...
def test_lwf_system():
    ...
    # ۱. پیدا کردن مسیر فایل lwf_model.pth
    current_dir = os.path.dirname(os.path.abspath(__file__))
    checkpoint_path = os.path.join(current_dir, "lwf_model.pth")
    
    if not os.path.exists(checkpoint_path):
        print(f"❌ فایل مدل یافت نشد: {checkpoint_path}")
        print("💡 لطفاً ابتدا اسکریپت train_lwf.py را اجرا کنید تا مدل آموزش دیده و فایل lwf_model.pth ساخته شود.")
        return

    # ۲. بارگذاری لیست اسامی کامل (۱۰۰۰ کلاس قدیمی + ۳ کلاس جدید)
    ...
    # ۳. بازسازی ساختار مدل ۱۰-۳ کلاسه دانش‌آموز
    print("📦 در حال بارگذاری مدل دانش‌آموز LwF...")
    ...
    print("\n✅ سیستم تست LwF آماده است!")
    print("=" * 65)

    # ۵. حلقه تعاملی پیش‌بینی تصاویر
    while True:
        image_path = input("\n🖼️ مسیر عکس برای تست را وارد کنید (یا عبارت exit برای خروج): ").strip()
        
        # پاک‌سازی گیومه‌های احتمالی دور مسیر فایل
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # استخراج ۵ حدس برتر (Top-5)
                top5_prob, top5_catid = torch.topk(probabilities, k=min(5, total_classes))

            print(f"\n🔍 ۵ حدس برتر مدل LwF برای تصویر: {os.path.basename(image_path)}")
            print("-" * 65)
