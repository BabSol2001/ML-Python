...
if os.path.exists(MODEL_PATH):
    print(f"\nبارگذاری فایل وزن‌ها ({MODEL_PATH})...")
    try:
        model.load_state_dict(torch.load(MODEL_PATH, map_location=device, weights_only=True))
        print("وزن‌ها با موفقیت بارگذاری شدند.\n")
    except Exception as e:
        print("خطا در بارگذاری وزن‌ها! لطفاً فایل deep_mini_gpt.pth را آموزش دهید.")
        sys.exit(1)
else:
    print("فایل وزن‌ها پیدا نشد. ابتدا برنامه اصلی را اجرا کنید تا وزن‌ها آموزش ببینند.")
    sys.exit(1)
