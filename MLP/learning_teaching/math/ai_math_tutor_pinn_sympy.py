...
# استفاده مستقیم از توابع و کلاس‌های فایل‌های قبلی بدون بازنویسی
from pinn import train_pinn, PINN1DHeat
from integration_test import exact_solution


class SocraticMathTutor:
    def __init__(self):
        # تعریف نمادها جهت ارزیابی جبر محاسباتی با SymPy
        self.x, self.t, self.L, self.alpha, self.n = sp.symbols('x t L alpha n', positive=True, real=True)
        self.pi = sp.pi
        
        self.symbols_dict = {
            'x': self.x, 't': self.t, 'L': self.L, 
            'alpha': self.alpha, 'n': self.n, 'pi': self.pi,
            'sin': sp.sin, 'cos': sp.cos, 'exp': sp.exp
        }

    def evaluate_sympy_expression(self, user_input, target_expr):
        """سنجش صحت ریاضی ورودی دانش‌آموز با استفاده از SymPy"""
        try:
            user_expr = sp.sympify(user_input, locals=self.symbols_dict)
            diff = sp.simplify(user_expr - target_expr)
            return diff == 0
        except Exception:
            return False

    def evaluate_verbal_concept(self, user_input, keywords):
        """سنجش صحت توضیحات کلامی دانش‌آموز بر اساس مفاهیم کلیدی"""
        user_input_lower = user_input.lower()
        # اگر حداقل یک مجموعه از کلمات کلیدی مفاهیم در متن موجود باشد
        return any(kw in user_input_lower for kw in keywords)

    def ask_socratic_step(self, title, question, target_expr, hints, concept_keywords=None, validation_type="sympy"):
        """اجرای یک گام تعاملی با پشتیبانی دوگانه از فرمول ریاضی و توضیحات کلامی"""
        print(f"\n============================================================")
        print(f"📌 {title}")
        print(f"============================================================")
        print(question)
        ...
        while True:
            ...
            # فاز ۱: ارزیابی مستقیم بر اساس نوع تعریف‌شده
            is_correct = False
            if validation_type == "sympy":
                is_correct = self.evaluate_sympy_expression(user_input, target_expr)
            elif validation_type == "keyword":
                is_correct = any(kw in user_input.lower() for kw in target_expr)

            # اگر ورودی یک عبارت ریاضی معتبر بود و درست بود
            if is_correct:
                print("\n✅ آفرین! پاسخ شما کاملاً درست است. استدلال ریاضی شیوایی داشتید.")
                break

            # فاز ۲: پشتیبانی از توضیحات کلامی (اگر عبارت ریاضی مستقیم نبوده باشد)
            if validation_type == "sympy" and concept_keywords:
                is_concept_understood = self.evaluate_verbal_concept(user_input, concept_keywords)
                if is_concept_understood:
                    print("\n💡 مفهوم منطقی کلام شما کاملاً درست است! افرین که استدلال فیزیکی/ریاضی مسئله را متوجه شدید.")
                    print("حالا همین مفهومی را که گفتی به‌صورت فرمول ریاضی بنویس (مثلاً: sin(n*pi*x/L)).")
                    continue

            # در صورت عدم تطابق ریاضی و کلامی
            attempt += 1
            ...
            if attempt == 1:
                ...
                print("اصلاً نگران نباش! هدف اصلی درک روند حل است. بریم سراغ گام بعدی.")
                break

    def run_pinn_verification_demo(self, user_final_expr):
        """اتصال به فایل‌های pinn.py و integration_test.py جهت شبیه‌سازی عملی"""
        print("\n============================================================")
        print("🤖 ورود به لایه هوش مصنوعی و شبیه‌سازی (PINN Verification)")
        print("============================================================")
        print("حالا که شما فرمول تحلیلی را استخراج کردید، می‌خواهیم شبکه عصبی فیزیک‌محوری را که")
        print("در فایل pinn.py تعریف کرده‌ایم آموزش دهیم و دقت فرمول شما را با هوش مصنوعی بسنجیم!\n")

        confirm = input("آیا مایلید شبکه عصبی را آموزش داده و نمودار مقایسه را تولید کنیم؟ (y/n): ")
        if confirm.lower() == 'y':
            model = train_pinn()
            ...
            plt.plot(x_test, T_exact, 'r-', label='Exact Analytical (Your Solution)', linewidth=2)
            plt.plot(x_test, T_pinn, 'b--', label='PINN Neural Network', linewidth=2)
            plt.title(f'Socratic Math AI: Exact vs Neural Net (MAE: {mae_error:.6f})')
...
شرط اولیه:  T(x,0) = sin(pi * x / L)
        ...
        # گام ۲: حل بخش مکانی (پشتیبانی از توضیحات کلامی مثل سینوس و...)
        target_X = sp.sympify("sin(n * pi * x / L)", locals=self.symbols_dict)
        self.ask_socratic_step(
        ...
        # گام ۳: حل بخش زمانی (پشتیبانی از توضیحات کلامی مثل نمایی و...)
        target_time = sp.sympify("exp(-alpha * (n * pi / L)**2 * t)", locals=self.symbols_dict)
        self.ask_socratic_step(
        ...
        target_final = sp.sympify("sin(pi * x / L) * exp(-alpha * (pi / L)**2 * t)", locals=self.symbols_dict)
        self.ask_socratic_step(
        ...
        # اجرای شبیه‌سازی تعاملی با استفاده از کد فایل‌های pinn.py و integration_test.py
